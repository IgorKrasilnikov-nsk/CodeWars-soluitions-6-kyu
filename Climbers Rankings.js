...
Climbers get points for each competition they attend, if they are in the top 30 (1st gets 100, 2nd: 80, 3rd: 65, 4th: 55, 5th: 51,...).
...
#Task: Create GetRankings function, that will take collection of climbers' names with their results, and return collection of climbers' names and sum of their best 6 results. 
Returned collection should be ordered by sum of results in descending order. With other words: climber with highest sum of points should be first, climber with lowest sum of points should be last.
...
class RankCalculator {
  static GetRankings(pointsByClimber) {
    const rankings = pointsByClimber.map(climber => {
      const bestPoints = climber.points
        .sort((a, b) => b - a)
        .slice(0, 6)
...
const result = RankCalculator.GetRankings(testData);
