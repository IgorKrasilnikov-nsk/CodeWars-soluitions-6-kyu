...
function birdCode(arr) {
  let res = [];
  for (let elem of arr) {
    let words = elem.replace(/-/g, " ").split(' ');
    let code = "";
    ...
    switch (words.length) {
      ...
      case 2:
        code = words[0].slice(0, 2).toUpperCase() + 
               words[1].slice(0, 2).toUpperCase();
        break;
      case 3:
        code = words[0].slice(0, 1).toUpperCase() +
               words[1].slice(0, 1).toUpperCase() + 
               words[2].slice(0, 2).toUpperCase();
        break;
      case 4:
        code = words[0].slice(0, 1).toUpperCase() + 
               words[1].slice(0, 1).toUpperCase() + 
               words[2].slice(0, 1).toUpperCase() + 
               words[3].slice(0, 1).toUpperCase();
        break;
      }
